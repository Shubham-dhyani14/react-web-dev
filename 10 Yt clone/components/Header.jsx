...
export default function Header() {
  ...
  return (
    <>
    <header className='fixed w-full top-0 p-1 py-2 sm:py-0 sm:p-2 md:p-4  shadow-sm  z-10  bg-white'>
        ...
        <nav className='flex justify-between items-center'>
            ...
            <div className='flex items-center'>
                <div className='w-8 h-8  p-1 text-center rounded-full cursor-pointer hover:bg-slate-300'
                onClick={toggleSidebar}
                >
                    <i className="fa-solid fa-bars fa-lg"></i>
                </div>
                <Link to={'/'}>
                <img className='h-6 ml-1 md:ml-4 cursor-pointer' src="https://www.gstatic.com/youtube/img/branding/youtubelogo/svg/youtubelogo.svg" alt="logo-Yt" />
                </Link>
            </div>
            {/* search bar */}
            ...
            <div className='flex gap-1 items-center'>
                ...
                <button className='bg-red-600 hover:bg-red-500 px-2  md:text-base py-1 text-gray-50 rounded-lg'><Link to={'/about'}>About</Link></button> 
                ...
                <div className={`absolute ${isMobileSearchShown ? 'block' : 'hidden'}   top-0 left-0 w-full flex gap-6  items-center sm:hidden p-2 bg-gray-100  border-gray-300`}>
                    <div onClick={()=>{setIsMobileSearchShown(false)}}  className='self-center ml-3'><i className="fa-solid fa-arrow-left"></i></div>
                    <div className=''>
                        
                    <input  onChange={(e)=>handleSearch(e.target.value)} id='search' placeholder='KhOJO...' className='w-64 ml-3 self-center px-2 py-1 flex-1 ring-2 rounded-full ring-gray-400 focus:outline-none focus:ring-blue-300 focus:border-0 search' type="text" />
                    </div>
                    {
                       ...
                       autoCompleteValues.length ?   <ul className={`absolute w-full  translate-y-full  rounded-b-md py-2 bottom-0 left-0 ring-1 ring-gray-400   bg-gray-100 shadow-md`}>
                            {
                                autoCompleteValues.map(val=>{return <li onClick={(e)=>handelSearchVideo(e.target.innerText)} className='cursor-pointer py-1 pl-8 pr-2 hover:font-medium hover:bg-slate-100 '>{val}</li>})
                            }
                        </ul> : false
                    }
                </div>
...
function Sidebar({sectionShown , toggleSidebar}){
    ...
    return (
    <> 
    <aside className= {`w-42  p-1 pr-5 py-2 sm:px-2 sm:py-0 md:p-4  ${!sectionShown && '-translate-x-48' }  transition-transform duration-300 bg-gray-200  bg-opacity-95  absolute top-0 left-0 h-screen`} >
        {/* logo and toogle-menu */}
        <div className='flex mt-0.5 items-center'>
                ...
                <img className='h-6 ml-1 md:ml-4' src="https://www.gstatic.com/youtube/img/branding/youtubelogo/svg/youtubelogo.svg" alt="logo-Yt" />
        </div>
        {/* menu options */}
        <ul className='mt-4'>
            <li className="hover:bg-slate-300 px-1 py-1 rounded-lg transition-all duration-200"  ><Link to="/" className='flex items-center'><span className="material-symbols-rounded w-6">home</span> <p className='ml-3 md:ml-5'>Home</p> </Link></li>


            <li  className="hover:bg-slate-300 px-1 py-1 rounded-lg transition-all duration-200" ><Link to="/lol"className='flex items-center '><span className="material-symbols-rounded w-6">movie</span> <p className='ml-3 md:ml-5'>Shorts</p> </Link></li>
            <hr />

            <li className="hover:bg-slate-300 px-1 py-1 rounded-lg transition-all duration-200" ><Link to="/lol"className='flex items-center '>
                <span className="material-symbols-rounded w-6">subscriptions</span><p className='ml-3 md:ml-5'>Subscriptions</p> </Link></li>
            <hr />

            <li className="hover:bg-slate-300 px-1 py-1 rounded-lg transition-all duration-200"  ><Link to="/lol"className='flex items-center  '><span className="material-symbols-rounded w-6">
video_library
</span><p className='ml-3 md:ml-5'>Library</p> </Link></li>
            <hr />
        </ul>
        {/* mr-4 md:mr-6 */}
    </aside>
